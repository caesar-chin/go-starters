...
func getExpenseReport(e expense) (string, float64) {
	// ?
	if report, ok := e.(email); ok {
		return report.toAddress, report.cost()
	} else if sms_report, sms_ok := e.(sms); sms_ok {
		return  sms_report.toPhoneNumber, sms_report.cost()
	} else {
		return "", 0.0
	}
}
